  ...
  public = list(
	...
	plotBERvsAc50 = function( label_by = "casn"){
      if (self$BERData$calcBERStatsDataExists() && self$basicData$basicStatsDataExists()) {
		...
		minv = min(min(basic_data$ac50),min(log10(BER_data$oral_ber)));
		maxv = max(max(basic_data$ac50),max(log10(BER_data$oral_ber)));
		if(label_by == "casn"){
			private$pltBERvsAc50 <- plot_ly() %>%
			  add_trace(data = BER_data, x = ~casn, y = ~signif(log10(oral_ber), digits = 5), type = 'box', name = 'BE',) %>%
			  add_trace(data = basic_data, x = ~casn, y = ~signif(ac50, digits = 5), type = 'box', name = 'Ac50', yaxis = 'y2') %>%
			  layout(title = 'Log Consumer Product Exposure vs  Log Ac50',
					 xaxis = list(title = ""),
					yaxis = list(range = c(minv - 1 , maxv + 1), side = 'left', title = 'Log Consumer Product Exposure', showgrid = FALSE, zeroline = FALSE),
					yaxis2 = list(range = c(minv - 1, maxv + 1), side = 'right', overlaying = "y", title = 'Log ac50', showgrid = FALSE, zeroline = FALSE, scaleanchor = "y", scaleratio = 1));
	
		}else{
			private$pltBERvsAc50 <- plot_ly() %>%
			  add_trace(data = BER_data, x = ~name, y = ~signif(log10(oral_ber), digits = 5), type = 'box', name = 'BE') %>%
			  add_trace(data = basic_data, x = ~name, y =~signif(ac50, digits = 5), type = 'box', name = 'Ac50', yaxis = 'y2') %>%
			  layout(title = 'Log Consumer Product Exposure vs Log Ac50',
					xaxis = list(title = ""),
					yaxis = list(range = c(minv - 1 , maxv + 1), side = 'left', title = 'Log Consumer Product Exposure', showgrid = FALSE, zeroline = FALSE),
					yaxis2 = list(range = c(minv - 1, maxv + 1), side = 'right', overlaying = "y", title = 'Log ac50', showgrid = FALSE, zeroline = FALSE, scaleanchor = "y", scaleratio = 1)
      ...
      if (self$BERData$calcBERStatsDataExists() && self$basicData$basicStatsDataExists()) {
		...
		if(label_by == "casn"){
			private$pltBER <- plot_ly() %>%
			  ...
			  layout(title = 'Log (ac50/Consumer Products Exposition)',
					 xaxis = list(title = ""),
					 yaxis = list(title = "Log (ac50/Consumer Products Exposition)"));
